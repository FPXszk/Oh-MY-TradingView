...
| universe | 対象銘柄のリスト | `config/backtest/universes/latest/*.json` |
| campaign | どの universe に対して、どの preset 群を、どの phase で回すかの実行定義 | `config/backtest/campaigns/latest/*.json` |
...
- 例:
  - `config/backtest/universes/latest/next-long-run-us-12.json`
  - `config/backtest/universes/latest/next-long-run-jp-12.json`
...
| 3 | `config/backtest/campaigns/latest/*.json` など | どの campaign を回すか |
| 4 | `config/backtest/universes/latest/*.json` など | campaign が指す銘柄群 |
...
| `docs/research/latest/main-backtest-latest-summary.md` | 最新 main backtest の要約 |
...
2. **この `docs/explain-forhuman.md`**  
   - 用語と全体像をつかむ
3. **`docs/research/latest/main-backtest-latest-summary.md`**  
   - 最新の勝ち筋と注意点を最短で確認する
4. **`docs/research/latest/next-long-run-us-jp-12x10-handoff_20260414_0009.md`**  
   - 今登録した 12×10 が何かを知る
5. **`docs/research/latest/next-long-run-us-jp-12x10-details_20260414_0009.md`**  
   - 10戦略 / 12銘柄の選定理由を確認する
6. **`docs/research/strategy/README.md`**  
   - 実際にどの戦略・銘柄を見ているかを確認する
7. **`.github/workflows/night-batch-self-hosted.yml`**  
   - 実際にどう起動するかを確認する
...
- latest handoff generation = 今いちばん新しく説明している docs 世代（現在は **12×10**）
- latest main backtest summary = 利用可能な最新 main artifact から再生成した summary（2026-04-15 時点では **finetune 100×10 smoke artifact**）
...
- campaign: `config/backtest/campaigns/latest/*.json`
- universe: `config/backtest/universes/latest/*.json`
