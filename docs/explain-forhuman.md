...
## 今回の追加探索: `emr-ae-v13-tp10-qty25` 派生 100戦略

### 何を見たいのか

前回の focus-8 バックテストでは、`emr-ae-v13-tp10-qty25` が
「勝ち銘柄を半分も早売りしない」方向で利益をかなり押し上げた一方、
`EMA20 trail` 自体の有効性はまだ十分に証明できていない。

今回の探索では、まず `EMA20 trail` を外し、
**初回利確の深さ (`tp1Pct`) と利確量 (`tp1Qty`) だけ** を 100 パターン比較する。

### 固定する条件

- 出来高条件: `volume = 20日平均の 1.3 倍以上`
- 長期トレンド条件: `価格が EMA200 より上`
- 初期ストップ: `-8% 固定`
- ブレイクアウト確認: `終値で 10日高値を上抜く`
- `EMA20 trail` は **使わない**

つまり、
「上昇トレンド中の出来高付きブレイクアウトだけを買い、
どの位置で・どれだけ早利確するのが一番マシか」
を切り分けるための探索。
...
- `v13`: 出来高フィルタ 1.3x
- `tp10`: 含み益 +10% で初回利確
- `q25`: その時点でポジションの 25% だけ利確
- `notrail`: 追加の EMA20 トレールは使わない

### 今回の比較で分かること

- `tp1Qty=25` でもまだ早売りしすぎか
- `tp1Pct=10` は浅すぎるのか、ちょうどよいのか
- 勝率を多少落としてでも net profit を伸ばせる組み合わせがあるか
- バイアンドホールドに少しでも近づくには、まず「何%で何割売るべきか」

この探索では volume / trend / stop をいじらない。  
先に TP と Qty の最適地形を見てから、必要なら次に trail や volume を再評価する。
