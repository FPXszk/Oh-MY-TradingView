...
> 旧既定の fine-tune 100×10 campaign（`next-long-run-us-finetune-100x10` / `next-long-run-jp-finetune-100x10`）は campaign 定義として残っています。手動で `config_path` を差し替えれば引き続き実行できます。
...
| `docs/command.md` | 実行コマンド・運用 runbook |
...
この `docs/explain-forhuman.md` は、**最初の入口**です。  
...
1. **この `docs/explain-forhuman.md`**  
   - 用語と全体像をつかむ
2. **`docs/research/latest/main-backtest-latest-summary.md`**  
   - 最新の勝ち筋と注意点を最短で確認する
3. **`docs/research/latest/next-long-run-us-jp-12x10-handoff_20260414_0009.md`**  
   - 今登録した 12×10 が何かを知る
4. **`docs/research/latest/next-long-run-us-jp-12x10-details_20260414_0009.md`**  
   - 10戦略 / 12銘柄の選定理由を確認する
5. **`docs/command.md` と `.github/workflows/night-batch-self-hosted.yml`**  
   - 実際にどう起動するかを確認する
...
### 1. 「latest docs」と「workflow 既定値」は現在同じ世代

今の latest docs も workflow 既定値も、どちらも **12×10** 世代です。  
ただしこの 2 つは常に同期が保証されるわけではありません。
