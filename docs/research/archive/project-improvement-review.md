...
今は `latest` に残す doc や config を人間が判断している。`docs/research/current/manifest.json` のような keep-list を置き、`scripts/docs/archive-stale-latest.mjs` がそれを読む形にすると、generation 更新時の事故を減らせる。

## 2. backtest ranking を毎回 artifact 化する

今回 strongest 15 を決める時、latest + previous だけでは自然候補が 14 本で止まった。`recovered-results.json` から **combined ranking JSON** を毎回生成し、`references/backtests/` に残すようにすると、次回の keep-set 更新が deterministic になる。

## 3. smoke-prod 完了後の summary を richer にする

`python/night_batch.py` で latest summary 自動生成は入れたが、まだ rich report 本体と ranking artifact は任意。workflow 完了後に `report` まで確実に実行し、`main-backtest-current-summary.md` と ranking JSON を同時更新する導線にすると handoff が強くなる。
...
`artifacts/` に統合したことで導線は改善したが、将来的には repo サイズが増える。長期的には
