...
| **latest_checkpoint** | smoke: `checkpoint-6.json` / full: `checkpoint-244.json` | ✅ full 完走を確認 |
...
#### A-1. Run 48 artifact 上の実行構成

Run 48 の `selected-us40-8pack` artifact では、以下の **5 つの Donchian-based finetune 戦略** が full 40 銘柄を完走した。右 2 列の performance 値は artifact ではなく strongest benchmark の参照値:

| # | 戦略 ID | composite_score | benchmark avg_net_profit | benchmark avg_profit_factor | 実行状況 |
| ---: | --- | ---: | ---: | ---: | --- |
...
- 合計: 205/205 = 100% 成功
- latest_checkpoint: smoke `checkpoint-8.json` / full `checkpoint-280.json`

#### A-2. Run 48 の metrics 可用性に関する注意

Run 48 の artifact 実測値と strongest ベンチマーク値は同一ではない。

- `selected-us40-8pack/full/recovered-results.json` では、上記 5 つの Donchian preset は **200/200 success** だが、metrics は `net_profit = 0`, `closed_trades = 0`, `profit_factor = null` で記録されている
- そのため、**Run 48 artifact 単体では strongest 5 本の profit 比較はできない**
- 以下の strongest ベンチマーク値は、既存の正本である `docs/research/current/main-backtest-current-summary.md` を出典として使う

#### A-3. 最強戦略 TOP 3 の詳細成績
...
| **実行失敗** | 0 件 | 0 件 | 🟰 同等 |
| **メトリクス読不可** | 0 件 | 0 件 | 🟰 同等 |
| **latest_checkpoint** | full `checkpoint-244.json` | full `checkpoint-280.json` | ✅ Run 48 |
| **non-zero metrics run** | 120/240 | 0/200 | ✅ Run 63 |

**結論**: 実行安定性は同等。加えて、Run 63 は full 240 run すべてで recovered metrics を取得できた。
...
#### C-1. strongest ベンチマーク値

`docs/research/current/main-backtest-current-summary.md` から取得した strongest 3 戦略の avg_net_profit:
...
#### C-2. Run 63 の実測パフォーマンス

Run 63 の `artifacts/campaigns/breakout-6pack-us40/full/recovered-results.json` から集計:

| 区分 | runs | avg_net_profit | avg_profit_factor | avg_max_drawdown | avg_win_rate | 補足 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| **Run 63 全体** | 240 | 65.04 | 1.3910 | 108.52 | 16.10% | 6 戦略全体。PF/win_rate は non-null 値で平均 |
| **Finder 3 本のみ** | 120 | 130.08 | 1.3910 | 217.04 | 16.10% | 実際に約定が発生した subset |
| **best avg_net_profit** | 40 | 151.11 | 1.3598 | 237.34 | 17.72% | `breakout-finder-balanced` |
| **best avg_profit_factor** | 40 | 128.93 | 1.5258 | 195.67 | 14.52% | `breakout-finder-wide` |
| **trend follower 3 本** | 120 | 0.00 | n/a | 0.00 | n/a | 40/40 ですべて `closed_trades = 0` |

`breakout-finder-balanced` の銘柄別 Top 5:

- META: net_profit `1401.46`, profit_factor `3.8578`
- SOXX: net_profit `1393.22`, profit_factor `4.3767`
- INTC: net_profit `1061.77`, profit_factor `2.4382`
- AMD: net_profit `803.32`, profit_factor `2.0687`
- SMH: net_profit `722.77`, profit_factor `2.5159`

`breakout-finder-balanced` の Bottom 5:

- USO: net_profit `-989.44`, profit_factor `0.3630`
- CRM: net_profit `-485.60`, profit_factor `0.3854`
- T: net_profit `-361.68`, profit_factor `0.2807`
- BA: net_profit `-333.73`, profit_factor `0.3181`
- NKE: net_profit `-278.54`, profit_factor `0.5468`
...
- **Run 63 best avg_net_profit**: `151.11` (`breakout-finder-balanced`)
- **Run 63 best avg_profit_factor**: `1.5258` (`breakout-finder-wide`)
- **strongest benchmark**: avg_net_profit `20,538.77`, profit_factor `1.762`
- **差分**: avg_net_profit は strongest US 上位に対して約 `-20,387.66`

**判定**: 実行完走性は高いが、profit performance は strongest 系にまだ大きく届いていない
...
2. 🔧 **Trend Follower 3 本の zero-trade 原因確認** (CRITICAL)
   - `breakout-trend-follower-fast / balanced / slow` が 40/40 で `closed_trades = 0`
   - entry 条件、期間、対象銘柄との適合性を確認
   - 約定が出ないまま完走しているため、安定性と性能が分離している
...
| **実行安定性** | ⭐⭐⭐⭐⭐ (100% 完走) |
| **戦略品質** | ⭐⭐⭐⭐ (新規設計、別系統) |
| **性能実績** | ⭐⭐ (best avg_net_profit 151.11) |
| **運用推奨度** | ✅ **推奨** (リスク分散用・安定性重視) |

### 最終判定

**🎯 Run 63 Breakout 6-Pack は「安定性では追随、性能では未追随」**

### 採用判断

**結論: 既存 strongest 戦略には採用しない。 strongest 側は現状維持とする。**

- `run63` から strongest へそのまま取り込める alpha 改善要素は、今回の実測値では確認できなかった
- `breakout-finder-balanced` の best avg_net_profit は `151.11` で、 strongest US 上位の `20,538.77` を大幅に下回る
- `breakout-trend-follower-fast / balanced / slow` は 40/40 で `closed_trades = 0` であり、exit 部品や補助ロジックとしても現時点では採用根拠が弱い
- 採用できるのはロジックではなく、失敗しやすい preset を default bundle から外して完走率を上げる運用判断だけであり、この点はすでに current workflow に反映済み
...
🟡 残課題:
1. Trend follower 3 本が zero-trade になった理由の特定
2. Finder 系 3 本の avg_net_profit を 2 桁から 4 桁以上へ引き上げる設計見直し
3. JP 市場への適用可能性
...
- **Run 63 full summary**: `/mnt/c/actions-runner/_work/Oh-MY-TradingView/Oh-MY-TradingView/artifacts/campaigns/breakout-6pack-us40/full/recovered-summary.json`
- **Run 63 full results**: `/mnt/c/actions-runner/_work/Oh-MY-TradingView/Oh-MY-TradingView/artifacts/campaigns/breakout-6pack-us40/full/recovered-results.json`
- **Run 48 full summary**: `/mnt/c/actions-runner/_work/Oh-MY-TradingView/Oh-MY-TradingView/artifacts/campaigns/selected-us40-8pack/full/recovered-summary.json`
- **本レポート生成**: 2026-04-23 15:10 JST
