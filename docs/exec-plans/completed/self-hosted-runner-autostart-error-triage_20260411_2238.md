...
`command.md` 末尾に貼られた Windows 実行ログでは、self-hosted runner の autostart 登録時に以下 3 系統の異常が見えている。
...
- `command.md` のエラーログと既存 Windows script の実装を突合し、3 事象を分離して原因を特定する
- RED -> GREEN -> REFACTOR で、ASCII-safe・`schtasks /TR` quoting・wrapper path 解決の回帰テストを追加する
- 必要に応じて `register-self-hosted-runner-autostart.cmd` と関連 wrapper を修正し、README / `command.md` の運用手順も更新する
...
- [ ] `command.md` 末尾ログと script 実装を突合し、文字化け / wrapper not found / schtasks invalid argument を個別に切り分ける
