...
- [x] ポートフォリオ診断レポート生成スクリプトを追加する。
- [x] 手動実行専用 workflow を追加する。
- [x] レポート生成ロジックの unit test を追加する。
- [x] `node --test tests/moomoo.test.js` と `git diff --check` を実行する。
- [x] 計画を completed に移動し、全変更を commit / push する。
