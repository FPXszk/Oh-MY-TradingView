...
volume / trend / stop は固定しつつ **TP の深さと利確量だけ**を切り分けること。
...
`emr-ae-v13-tp{TP}-q{QTY}-notrail`
...
| `tests/windows-run-night-batch-self-hosted.test.js` | 新 night_batch config の存在と campaign 参照先を確認 |
| `docs/explain-forhuman.md` | 今回の 100戦略探索の目的と戦略概要を末尾追記 |
...
4. レイアウト系テストと catalog / workflow config テストの期待値を調整し、登録漏れを検知できる状態に保つ
5. `docs/explain-forhuman.md` に今回の探索の狙いと戦略の見方を追記する
...
- [x] `emr-ae-v13-tp10-qty25` の現行定義をテンプレートとして再確認し、生成対象パラメータを `tp1Pct` と `tp1Qty` のみに限定する
- [x] `scripts/generate-emr-ae-tpqty-100pack.py` を追加し、100 本の Pine / preset / catalog 定義を再生成可能にする
- [x] `docs/references/pine/emr-ae-tpqty-100pack/` に 100 本の Pine を出力する
- [x] `config/backtest/strategy-presets.json` に 100 preset を追加する
- [x] `config/backtest/strategy-catalog.json` に 100 catalog entry を追加する
- [x] `config/backtest/campaigns/emr-ae-tpqty-100pack-focus8.json` を追加し、focus-8 / 2015-01-01〜2026-04-27 / smoke=SPY / full=8 symbols を設定する
- [x] `config/night_batch/emr-ae-tpqty-100pack-focus8-config.json` を追加し、workflow dispatch から使えるようにする
- [x] `tests/repo-layout.test.js` と `tests/strategy-catalog.test.js` の期待値と ID 一覧を更新する
- [x] `tests/windows-run-night-batch-self-hosted.test.js` に新 config の整合チェックを追加する
- [x] `docs/explain-forhuman.md` の末尾に今回の TP/QTY 探索の目的・固定条件・見たい論点を追記する
- [x] `node --test tests/repo-layout.test.js tests/strategy-catalog.test.js tests/windows-run-night-batch-self-hosted.test.js tests/strategy-live-retired-diff.test.js` で登録系の回帰確認を行う
- [x] `gh workflow run night-batch-smoke.yml --ref main -f config_path=config/night_batch/emr-ae-tpqty-100pack-focus8-config.json` を実行し、workflow run `25266246286` が作成されることを確認する
...
- `EMA20 trail` を外すため、利益の伸びは確認しやすくなる一方で、含み益の吐き出しは増える可能性がある
