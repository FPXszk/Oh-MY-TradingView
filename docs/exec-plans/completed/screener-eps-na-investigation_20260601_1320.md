...
- [x] 直近レポートで `SNDK` と `EPS YoY = N/A` 銘柄を列挙し、現象を固定する。
- [x] `src/core/fundamental-screener.js` で TradingView 取得列と Moomoo fallback 条件を確認する。
- [x] `src/core/moomoo.js` とテストで、Moomoo が補完できる条件と補完できない条件を確認する。
- [x] `docs/strategy/*` と既存メモから、両指標を direct 対応と見なした根拠と未検証点を洗い出す。
- [x] 可能なら比較プローブや既存データから、同一銘柄で TradingView 値と moomoo 値がどれだけズレるかを実測する。
- [x] 差が「単なる欠損」ではなく「定義差」の疑いかどうかを整理し、混在利用を避けるべき条件を報告する。
- [x] 必要ならローカル再実行で該当銘柄の最新出力を再確認する。
- [x] 最終的に、`SNDK` の負値理由、`N/A` 銘柄ごとの欠損理由、Moomoo 補完が効く/効かない境界を報告する。
