...
- [x] 登録スクリプトを実行して実ランチャーを再生成した。タスクの削除・再登録は権限不足で失敗したが、タスクが参照する既存ランチャーは更新済みである。
- [x] ログオン用ランチャーから OpenD 起動を削除し、関連する契約テストを更新した。
- [x] デスクトップの手動起動バッチを作成した。
- [x] 実ランチャーに `moomoo_OpenD.exe` 起動処理がないことを確認した。
- [x] 9226 が未使用であること、OpenD と Moomoo 実行ファイルの解決先が存在することを確認し、バッチで実アプリを起動した。`/json/version` が Moomoo の CDP 応答を返した。
- [x] `node --test tests/windows-run-night-batch-self-hosted.test.js` を実行し、70 件すべて成功した。
