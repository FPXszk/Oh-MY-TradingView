...
- [x] active 計画を作成する。
- [x] 計画だけを `docs: plan Japanese trade decision gate smoke test` で commit / push する。
- [x] `.agents/skills/trade-decision-gate/SKILL.md` を日本語化する。
- [x] `tests/skills-contract.test.js` を日本語仕様へ更新する。
...
- [x] `codex --help` と、存在する場合は非対話実行 help を確認する。
- [x] 新しい Codex CLI プロセスでの NVDA smoke を試行し、起動失敗理由、stdout / stderr / git status を保存する。
- [x] 同一セッション fallback smoke の取得データ、出力形式、read-only、最終判定を `validation.md` に記録する。
- [x] 差分をセルフレビューする。
- [x] active 計画へ実装結果、通常テスト結果、NVDA smoke 結果を追記する。
- [x] 計画を `docs/exec-plans/completed/` へ移動する。
- [x] 実装差分を `fix: localize trade decision gate and verify NVDA smoke` で commit / push する。
- [x] `git pull origin main` と `git status --short --branch` で最終同期と clean tree を確認する。
...
- `.agents/skills/trade-decision-gate/SKILL.md` の front matter `description`、本文、見出し、起動条件、起動対象外、正本、判断モード、判定優先順位、モード別ワークフロー、情報源、最新 screener run、TradingView 利用方針、画像入力、情報不足、ラベル、read-only、出力形式を日本語化した。
- `trade-decision-gate`、`SKILL.md`、`GO / STAY / STOP`、`NEW_ENTRY`、`ADD_POSITION`、`HOLD_OR_EXIT`、`PORTFOLIO_RISK`、`RISK_ON / NEUTRAL / RISK_OFF`、`FAILED_BREAKOUT`、`SETUP_BROKEN`、`NO_TRADE`、`COMPLETE / PARTIAL / INSUFFICIENT`、`US / JP / MIXED` は維持した。
- workflow 名、CLI command、file path、JSON field、Moomoo / TradingView tool 名は変更していない。
- `tests/skills-contract.test.js` は英語 heading と `is prohibited` 依存を削除し、日本語 description、日本語 heading、日本語 read-only 禁止表現、STOP 優先、HOLD / PORTFOLIO label、最新 success run route を検証する形へ更新した。
- `README.md`、`docs/strategy/Trade-rule.md`、operator skill、GitHub Actions debugging skill、documentation navigation test、既存 MCP / CLI / screener / Moomoo / TradingView CDP 実装は変更していない。
...
結果: pass。6 tests / 1 suite / 0 fail。

```powershell
node --test tests/documentation-navigation.test.js
```

結果: pass。3 tests / 1 suite / 0 fail。

```powershell
npm run test:unit
```

結果: pass。864 tests / 205 suites / 0 fail。

通常の `npm run test:e2e` は実行していない。理由は、今回は Markdown skill、contract test、smoke artifact の変更であり、TradingView Desktop / CDP 実機挙動を変更していないため。

## NVDA smoke 結果

- 保存先: `artifacts/trade-decision-gate-smoke/20260709_1938/`
- prompt: `prompt.txt`
- stdout: `stdout.txt`
- stderr: `stderr.txt`
- validation: `validation.md`
- OHLCV summary: `market-chart-summary.json`
- 独立 Codex CLI: 未実行。`codex.exe` が WindowsApps 配下で `アクセスが拒否されました` となり、`codex --help` / `codex exec --help` を起動できなかった。
- 同一セッション smoke: 実施。
- 取得できた主なデータ: NVDA 2026-07-08 日足 OHLCV、NVIDIA Q1 FY2027 公式 IR、SEC 10-Q、直近ニュース、Wall Street Horizon の次回決算候補、GitHub Actions 最新成功 screener run、repo screener report / metadata、SPY / QQQ / SMH / AMD / AVGO / MU / TSM 日足 summary。
- 取得できなかった主なデータ: Moomoo quote / TA / K-line、独立 Codex CLI の自動 skill 選択、TradingView 目視 pivot、口座情報、NVIDIA IR events page 上の次回決算日。
- データ完全性: `PARTIAL`
- 最終判定: `STAY`
- read-only: 合格。注文、アラート、口座書き込み、取引ロック解除は行っていない。

## レビュー結果

- skill 本文は自然な日本語に寄せ、既存の判断ロジックは維持した。
- 技術識別子、path、workflow、command、tool 名は変更していない。
- contract test は全文一致ではなく、重要契約に絞って日本語仕様を検証している。
- `HOLD_OR_EXIT + GO` 不使用、`PORTFOLIO_RISK + GO` が個別銘柄の購入許可ではないこと、確認済み `STOP` 条件が情報不足 `STAY` より優先されることを維持した。
- `Trade-rule.md`、operator skill、GitHub Actions debugging skill、他 skill の一括翻訳は行っていない。
