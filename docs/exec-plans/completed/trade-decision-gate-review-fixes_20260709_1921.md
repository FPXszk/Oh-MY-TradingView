...
- [x] 計画書を `docs/exec-plans/active/` に作成する。
- [x] 計画書のみを `docs: trade decision gate review fixes plan` で commit / push する。
- [x] `.agents/skills/trade-decision-gate/SKILL.md` をモード別ワークフロー、判定優先順位、ラベル仕様、出力順序、最新成功 run 経路に合わせて更新する。
- [x] `tests/skills-contract.test.js` の trade decision gate 契約テストを強化する。
...
- [x] 計画書を `docs/exec-plans/completed/` へ移動し、完了レビューと検証結果を追記する。
- [x] 完了差分を Conventional Commits で commit / push する。
- [x] `git pull origin main` と `git status --short --branch` で remote/local 同期と clean tree を確認する。
...
- `.agents/skills/trade-decision-gate/SKILL.md` を共通 19 ステップから、`NEW_ENTRY` / `ADD_POSITION` / `HOLD_OR_EXIT` / `PORTFOLIO_RISK` のモード別ワークフローへ整理した。
- 確認済み `STOP` 条件を情報不足 `STAY` より優先する判定順序を明記した。
- `HOLD_OR_EXIT + GO` を使わず、保有判断は保有継続の `STAY` と縮小・売却・損切り側の `STOP` で扱う仕様を明記した。
- `PORTFOLIO_RISK` で単一銘柄のピボット、エントリーゾーン、初期リスクリワード、全保有銘柄の完全な決算日・スクリーナー順位を無条件必須にしないことを明記した。
- `対象市場` に `US / JP / MIXED` を追加し、米国株と日本株が混在する場合の扱いを明記した。
- 出力順序を `# 判定: GO / STAY / STOP` が先頭に来る形へ修正し、`HOLD_OR_EXIT` と `PORTFOLIO_RISK` の最低限の出力項目を追加した。
- 最新成功 screener run の確認経路として GitHub connector / GitHub API tool、`gh run list`、repo metadata と report body の順序を明記した。
- `tests/skills-contract.test.js` に Markdown セクション抽出 helper を追加し、read-only、判定優先、モード別ワークフロー、ラベル仕様、出力順序、最新 run 経路を contract test 化した。

## レビュー結果

- 変更ファイルは計画対象の `.agents/skills/trade-decision-gate/SKILL.md`、`tests/skills-contract.test.js`、計画書の移動・追記に限定した。
- `README.md`、`docs/strategy/Trade-rule.md`、operator skill、GitHub Actions debugging skill、`tests/documentation-navigation.test.js` は変更していない。
- read-only セクションから、注文操作を許可しているように見え得る英語表現を除去した。
- contract test は全文一致ではなく、判断契約上の重要セクションと必須文言に絞った。
- E2E は TradingView Desktop / CDP / Moomoo / SBI / screener workflow の実装を変更していないため実行していない。
