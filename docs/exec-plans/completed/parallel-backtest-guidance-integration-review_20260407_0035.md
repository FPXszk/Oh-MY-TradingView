...
4. runbook を正式な運用方針のソースオブトゥルースにし、`command.md` を quick reference に揃える
...
- modified: `command.md`
- modified: `docs/design-docs/dual-worker-parallel-backtest-runbook_20260406_0735.md`
...
- runbook と `command.md` を最小修正し、最終順位・checkpoint の位置づけ・partial retry の非推奨条件を一貫して読める状態にする
...
- `command.md` が runbook の詳細を重複しすぎる可能性がある
...
- [ ] `command.md` の差分をレビューし、最終順位・partial retry 記述の曖昧さを確認する
- [ ] runbook の差分をレビューし、durable guidance として不足または過剰な記述を確認する
- [ ] runbook を必要最小限で修正し、正式な運用方針のソースオブトゥルースにする
- [ ] `command.md` を必要最小限で修正し、runbook と矛盾しない quick reference に揃える
...
- `command.md` が runbook と矛盾しない
- checkpoint と partial retry の位置づけが誤読されない
- 今後の運用で **runbook を主、`command.md` を従** として参照できる状態になっている
