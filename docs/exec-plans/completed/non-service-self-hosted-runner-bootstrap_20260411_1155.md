...
一方で repository 内の README / docs/command.md / workflow 周辺には、その非 service 前提がまだ十分に明文化されておらず、将来の運用者が古い前提を参照して誤認する余地がある。
...
- `README.md` と `docs/command.md` に、**service mode 非採用**・その理由・現在の運用前提・bootstrap 導線の概要が明記される
...
- README / docs/command.md / workflow / wrapper / test の **前提整合確認**
...
- その一回の導線変更手順を README / docs/command.md に明記する
...
- README は **運用方針と前提**、docs/command.md は **手順とコマンド** を中心に役割分担する
...
- README / docs/command.md の重複説明を削減する
...
npm test
git --no-pager diff -- README.md docs/command.md .github/workflows/night-batch-self-hosted.yml scripts/windows tests/windows-run-night-batch-self-hosted.test.js docs/working-memory/session-logs
```

必要に応じて文言確認:

```bash
rg -n "service|run\\.cmd|bootstrap|self-hosted" README.md docs/command.md .github/workflows/night-batch-self-hosted.yml scripts/windows tests/windows-run-night-batch-self-hosted.test.js
```
...
2. **README / docs/command.md / workflow の説明がずれる可能性**  
   - 対策: README は方針、docs/command.md は手順、と役割を固定する
...
- [ ] docs/command.md の self-hosted 節に追記する bootstrap 先行手順・manual hookup 記載案を固める
