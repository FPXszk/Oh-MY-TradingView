...
- [x] `src/core/sector-momentum.js` に benchmark/SPY 取得、breadth 集計、ranking formula 更新を実装する
- [x] `scripts/screener/run-fundamental-screening.mjs` の Phase1 表と説明文を新指標に合わせて更新する
- [x] `docs/reports/screener/TEMPLATE.md` を新しい Phase1 表示に合わせて更新する
- [x] `tests/daily-screener-report.test.js` を更新し、Markdown 出力の期待値を新仕様に合わせる
- [x] 対象テストを実行し、必要なら実レポートを再生成して生成物を確認する
