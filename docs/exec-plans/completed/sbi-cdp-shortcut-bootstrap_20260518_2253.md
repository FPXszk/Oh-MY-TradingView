...
- dry-run workflow で target 判定が妥当か確認する
...
- endpoint が生えても unrelated tab を SBI target と誤認する可能性がある
...
- dry-run workflow で `No SBI target` かそれに準ずる妥当な結果が出ること

## Steps

- [x] Step 1: Windows 側の Chrome パス、Desktop パス、既存 Chrome 状況を確認する
- [x] Step 2: 既存セッションを壊さない CDP 起動方式を決める
- [x] Step 3: Desktop に CDP 用ショートカットを作成し、実行する
- [x] Step 4: `json/version` / `json/list` が返るか確認する
- [x] Step 5: dry-run workflow を実行し、必要なら target 判定の最小修正を入れる
- [x] Step 6: session log と必要な docs を更新する
- [x] Step 7: 変更をレビューし、plan を completed へ移して commit / push する
...
- Windows Desktop に dedicated CDP Chrome shortcut を作成できた
- Windows 側 `127.0.0.1:9222` では endpoint 応答を確認できた
- self-hosted Windows runner からも endpoint probe success を確認できた
- `about:blank` を SBI target と誤認する bug を見つけ、`score=0` target を除外する最小修正を入れた
- 次 blocker は `SBI tab 不在` まで前進した
