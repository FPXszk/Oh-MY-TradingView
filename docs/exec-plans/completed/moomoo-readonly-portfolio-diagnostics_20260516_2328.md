...
- [x] adapter に read-only trade context と account commands を追加する。
- [x] core に payload validation と public functions を追加する。
- [x] portfolio diagnostics の集計ロジックを追加する。
- [x] MCP tools と server instructions を追加する。
- [x] unit tests を追加・更新する。
- [x] live OpenD で read-only smoke を実行する。
- [x] 計画を completed に移動し、実装を commit / push する。
