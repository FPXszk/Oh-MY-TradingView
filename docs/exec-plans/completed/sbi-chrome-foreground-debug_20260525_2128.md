...
- [x] Windows 実機で使える Chrome 前面化手段を単体で確認する
- [x] 前面化 helper を repo に追加する
- [x] SBI capture から helper を呼ぶ最小接続を入れる
- [x] 必要なら workflow 側へ明示的な接続を追加する
- [x] テストを追加・更新する
- [x] 実機 workflow を `enable_sbi=true` で再実行して確認する
...
- `scripts/windows/focus-chrome-window.ps1` を追加し、`WScript.Shell.AppActivate` に加えて `ShowWindowAsync` / `BringWindowToTop` / `SetForegroundWindow` / `AttachThreadInput` / `SendKeys('%')` を組み合わせた Windows 前面化 helper を実装した
- `scripts/sbi/capture-portfolio-data.mjs` の `ensureSbiTargetActive()` から OS-level foreground helper を呼ぶようにし、CDP target activation より前に Windows 側の Chrome ウィンドウ前面化を試す形にした
- `tests/sbi-capture-workflow.test.js` に helper 呼び出し契約と helper 実装契約を追加した
- 単体検証では `beforeTitle=Codex` から `afterTitle=配当金・分配金履歴｜SBI証券 - Google Chrome` への前面化成功を確認した
- live workflow は 3 回確認した
  - `26400638788` は初回前面化 helper 追加版で success
  - `26401033201` は false negative 抑制版で success
  - `26401383887` は `AttachThreadInput` などを加えた最終版で success
- 最終 run `26401383887` の artifact `capture/latest/capture-summary.json` では `OS foreground helper succeeded: 配当金・分配金履歴｜SBI証券 - Google Chrome` が残り、foreground failure note は消えた
