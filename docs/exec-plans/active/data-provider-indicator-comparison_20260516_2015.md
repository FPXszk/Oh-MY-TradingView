...
- [ ] 既存実装と docs から、現行スクリーニングで使う指標を洗い出す。
- [ ] TradingView / Moomoo / Yahoo Finance の取得可能指標を実査または実装根拠で分類する。
- [ ] ファンダメンタル対応表、テクニカル対応表、運用結論を docs/strategy に作成する。
- [ ] ドキュメント内容をレビューし、過剰な断定や未確認事項の混入がないか確認する。
- [ ] 計画ファイルを completed に移動し、ドキュメントと計画を commit / push する。
