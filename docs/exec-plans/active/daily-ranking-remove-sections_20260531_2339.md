...
- [ ] `buildMarkdown()` のどこで 3 セクションを出しているかを整理し、最小変更の削除方針を確定する。
- [ ] `scripts/screener/run-fundamental-screening.mjs` を更新し、US 日次レポートで対象 3 セクションを出さないようにする。
- [ ] `tests/daily-screener-report.test.js` を更新し、不要セクションの absence を検証する。
- [ ] `docs/reports/screener/TEMPLATE.md` の章立てを更新する。
...
- [ ] `node --test tests/daily-screener-report.test.js` を実行する。
