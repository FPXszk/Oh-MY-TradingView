...
- [ ] 既存ドキュメント構成と現行スクリーナーの取得フィールドを確認する。
- [ ] Rule of 40 の定義、式、評価、データ取得可否を既存の指標表へ追加する。
- [ ] スクリーニングワークフローへの適用方法を追記する。
- [ ] 有効性と限界を、投資リターン因子ではなく SaaS valuation / operating quality proxy として整理する。
- [ ] 参考文献を追加し、Markdown 表の崩れがないか確認する。
- [ ] 計画ファイルを completed に移動し、変更をコミットして push する。
