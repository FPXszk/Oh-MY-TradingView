...
- [ ] live artifact と既存 code を突き合わせて、`n/a` メタ情報の欠落箇所を特定する
- [ ] 米国株 route の summary / snapshot / report 出力を確認し、仕様制約か不具合かを判定する
- [ ] 修正可能な箇所だけを最小変更で実装する
- [ ] unit test を追加または更新して挙動を固定する
- [ ] 必要なら workflow 手順書と session log を更新する
- [ ] plan を `completed/` へ移動して commit / push する
