...
- [ ] SBI証券の公式API / 開発者サイト / 法人・個人向け提供有無を調査する。
- [ ] 家計簿・金融データAPI・証券API系サービスのSBI証券連携方式を調査する。
- [ ] スクレイピング / RPA / OSS実装例と利用規約上のリスクを調査する。
- [ ] CSV手動エクスポートと他社証券API事例を調査する。
- [ ] `docs/research/sbi_portfolio_api.md`, manifest, 参照台帳を更新する。
- [ ] Markdown / JSON の整合性を確認し、計画を completed に移動して commit / push する。
