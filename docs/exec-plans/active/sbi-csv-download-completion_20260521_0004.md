# SBI CSV Download Completion Plan

## Goal

`SBI Portfolio Capture` workflow を、`CSVダウンロード` button の live click 検証で止めず、`実現損益詳細` と `配当金・分配金履歴` の CSV 本体が artifact `downloads/` に保存されるところまで完了させる。
...
- 原因に合わせて capture script の submit / navigation / download wait を最小限修正する
- 必要な unit test を追加して挙動を固定化する
- live workflow run を回して artifact で完成度を確認する
...
- `capture-summary` だけで「button が見えた」から「何が送信され、なぜ download しなかったか」まで追えるようになる
- runner 上の live debugging が短い反復で回せるようになる
- CSV 本体が取得できれば、既存 report builder の実現損益 / 履歴セクションがより完全になる

## Test Strategy

- `tests/sbi-capture-workflow.test.js` を先に RED 化し、submitter 情報や route diagnostic 出力の期待値を追加する
- 実装して GREEN にする
- 必要な場合のみ `tests/sbi-portfolio-report.test.js` を追加更新し、新規 CSV 反映または補助artifact 表示を固定する
- 最後に live workflow run を実行して artifact で実地確認する
...
- [ ] `scripts/sbi/capture-portfolio-data.mjs` に route ごとの CSV button / form diagnostic 採取を追加し、artifact から submit 条件を読めるようにする
- [ ] `tests/sbi-capture-workflow.test.js` を更新し、diagnostic summary と submit 条件の出力を固定化する
- [ ] live workflow run を実行し、artifact から `実現損益詳細` / `配当金・分配金履歴` の download 不発原因を特定する
- [ ] 原因に合わせて capture script の submit / navigation / download wait を最小変更で修正する
- [ ] 必要なら report builder / report test / 手順書 / durable session log を更新する
- [ ] live workflow run を再実行し、追加 CSV が `downloads/` に落ちるか、または到達不能理由が十分具体的に残ることを確認する
