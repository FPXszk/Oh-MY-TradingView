...
2026-05-14 時点では、この Phase 2 の最小実装として `moomoo_stock_filter_fields`、`moomoo_plate_breadth`、`moomoo_ohlc_compare`、`moomoo_screening_validate`、`moomoo_fundamental_probe` を追加し、OpenD 実機で US semiconductors plate と `US.NVDA` / `US.AVGO` / `US.AMD` を使った validation を完了した。`moomoo_screening_validate` は `benchmark` / `moomoo-only` を分離し、`SUM_OF_BUSINESS_GROWTH` は TradingView revenue growth proxy としてかなり近い一方、`DEBT_ASSET_RATE` は debt/equity の代替にはならず、`PCF_TTM` は partial proxy に留まることを確認した。詳細な field 対応表と live 結果は `docs/strategy/moomoo_phase2_screening_validation.md` を参照。
...
- `get_stock_filter()` で現行スクリーナーが重視する全指標をどこまで再現できるかは一部判明したが、`FCF margin`、`EV/EBITDA`、`beta`、`debt_to_equity` の exact mapping は未解決
- `SUM_OF_BUSINESS_GROWTH` は semi leader では revenue growth proxy として近かったが、他 sector / market で同じ精度か未検証
- `PCF_TTM` は `raw / 100` で TradingView `price_free_cash_flow_ttm` に近づくが、exact replacement には未達
- `request_history_kline()` の長期間ページング、adjustment 差、extended time 差分の検証は未了。現状の OHLC drift 比較は Yahoo daily series を基準にした近似確認に留まる
- Yahoo `quoteSummary` が 2026-05-14 実査で `401` を返し、現行 Yahoo fundamentals enrichment の可用性が不安定
