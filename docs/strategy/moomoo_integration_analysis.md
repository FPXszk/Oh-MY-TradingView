...
2026-05-14 時点では、この Phase 2 の最小実装として `moomoo_stock_filter_fields`、`moomoo_plate_breadth`、`moomoo_ohlc_compare`、`moomoo_screening_validate` を追加し、OpenD 実機で US semiconductors plate と `US.NVDA` / `US.AVGO` / `US.AMD` を使った validation を完了した。詳細な field 対応表と live 結果は `docs/strategy/moomoo_phase2_screening_validation.md` を参照。
