...
night batch のアーティファクト（log / summary / detached state）を `results/night-batch/roundN/` に整理する仕組み。完了した round は `results/night-batch/archive/roundN/` に自動退避される。`--round-mode` を指定しない場合は従来どおり `results/night-batch/` 直下にフラット出力される。
...
# アーカイブ済みラウンドの状態確認
ls results/night-batch/archive/round*/round-manifest.json
...
引数 2 を省略した wrapper は、完了済み round を `results/night-batch/archive/roundN/` に退避したうえで、`round-manifest.json` が残っている**アクティブ** round があれば `resume-current-round`、無ければ `advance-next-round` を自動選択する。

workflow は `.github/workflows/night-batch-self-hosted.yml` にあり、既定 cron は **毎日 00:00 JST**（`0 15 * * *` UTC）。  
想定 runner は **self-hosted Windows** で、既定 config は `config/night_batch/bundle-foreground-reuse-config.json`。runner が online であれば動作し、service 常駐は前提としない。workflow は smoke と production を **完了まで監視**し、GitHub Actions の success/failure を production 完了結果に揃える。workflow では `checkout clean: false` を維持しつつ、終了時に **`GITHUB_STEP_SUMMARY`** へ要約を書き出し、**`actions/upload-artifact`** で最新 round 成果物を回収し、完了後に round を `results/night-batch/archive/roundN/` へ退避する。**00:00 JST の起動窓を外れた stale scheduled run は skip** する。
...
以下は operator が現場から `command.md` 末尾へ貼り付けた **paste transcript**。  
