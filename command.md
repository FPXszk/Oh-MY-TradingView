...
`config/night_batch/bundle-detached-reuse-config.json` は、既存 `next-long-run-us-finetune-100x10` / `next-long-run-jp-finetune-100x10` を参照して **smoke -> full detached** を流す本番用 config。strategy ID を JSON に複製しない。  
...
7. `results/night-batch/` へ summary / log / detached state を出力（`--round-mode` 指定時は `results/night-batch/roundN/` へ）
...
python3 python/night_batch.py smoke-prod \
  --config config/night_batch/bundle-detached-reuse-config.json \
...
└── detached-production-state.json
```
