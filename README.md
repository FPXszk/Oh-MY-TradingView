...
- **`market_*` は外部取得あり**: `market_quote` / `market_fundamentals` / `market_snapshot` / `market_news` / `market_screener` / `market_ta_summary` / `market_ta_rank` / `market_symbol_analysis` は Yahoo Finance の public endpoint を使います
...
- `market_symbol_analysis` — 単一銘柄の deterministic analyst-style analysis（trend / fundamentals / news / risk / overall）
...
- `tv launch-browser [--port 9222] [--path /path/to/chrome] [--url https://www.tradingview.com/chart/] [--dry-run]`
...
src/
  ...
  cli/
    ...
    commands/
      ...
      market-intel.js # CLI: market quote/fundamentals/snapshot/news/screener/ta-summary/ta-rank/analysis
