...
- **要ユーザー起動**: TradingView Desktop を `--remote-debugging-port=<port>` 付きで起動しておく必要があります（推奨の visible-first default は `9225`）
...
- `tv launch [--port 9225] [--path /path/to/tv] [--dry-run]`
- `tv launch-browser [--port 9225] [--path /path/to/chrome] [--url https://www.tradingview.com/chart/] [--dry-run]`
...
Windows（visible-first default: `9225`）:

```cmd
"%LOCALAPPDATA%\TradingView\TradingView.exe" --remote-debugging-port=9225
```

必要なら次も試してください。

```cmd
"%LOCALAPPDATA%\TradingView\TradingView.exe" --remote-debugging-port=9225 --remote-debugging-address=0.0.0.0
```

> 推奨運用は **Windows 側で visible に開いた `9225` 個体へ attach** です。WSL からは `localhost` が届かないことがあるため、通常は Windows host IP 経由（例: `172.31.144.1:9225`）を使います。`9225` が応答しない場合、CLI の default は他ポートへ暗黙フォールバックしません。
...
夜間自動化は **Python が visible 9225 セッションの preflight を行い、TradingView 操作本体は既存 Node script を subprocess 実行する** 構成を想定しています。CDP/backtest 本体を Python に再実装しません。
...
Python スクリプトは `results/night-batch/` に run summary と log を残します。`bundle` / `campaign` / `recover` / `report` / `nightly` をサポートし、CDP が必要なコマンドでは 9225 preflight が通らない限り停止します。
```
