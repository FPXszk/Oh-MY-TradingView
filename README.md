...
- **`market_*` は外部取得あり**: `market_quote` / `market_fundamentals` / `market_snapshot` / `market_news` / `market_screener` / `market_ta_summary` / `market_ta_rank` は Yahoo Finance の public endpoint を使います
...
src/
  ...
  cli/
    ...
    commands/
      ...
      market-intel.js # CLI: market quote/fundamentals/snapshot/news/screener/ta-summary/ta-rank
