...
- `summary.json`
  ...
  - `last_checkpoint: docs/research/results/campaigns/next-long-run-jp-finetune-100x10/full/checkpoint-1006.json`
...
あわせて、run 8 の結果を **docs 配下の専用レポート**に整理し、`README.md` / `docs/command.md` から辿れるようにします。
...
- `tests/windows-run-night-batch-self-hosted.test.js` を拡張し、少なくとも以下を固定化する
  ...
  - README / docs/command.md から run 8 専用レポートへ辿れること
...
- レポートには以下を含める
  ...
  - 今回入れた再発防止策の要約
- `README.md` / `docs/command.md` に、run 8 結果レポートへの導線を追加する
...
- 失敗条件の例
  ...
  - README / docs/command.md から run 8 レポートへ辿れない
...
- run 8 レポートと README / docs/command.md の導線を追加し、テストを通す

## REFACTOR
- workflow YAML の責務を「配線」に限定し、PowerShell 実装詳細を script へ集約する
- step 名・script 名・引数名を読みやすく揃える
- docs は重複説明を避け、README / docs/command.md は短い案内、詳細は専用レポートへ寄せる
...
- [ ] `docs/command.md` に run 8 結果レポートへの参照と運用上の注意を追加する
- [ ] `npm test` で関連回帰を確認する
...
rg -n "night-batch-self-hosted-run8\.md" README.md docs/command.md docs/reports/night-batch-self-hosted-run8.md
```
...
- run 8 専用レポートが `README.md` / `docs/command.md` から辿れる
...
- `README.md` / `docs/command.md` から専用レポートへ辿れる
- `npm test` が通る
