...
ユーザーが直前に確認した内容、つまり以下を **非エンジニアでも追える 1 枚の説明文書** に整理し、repo 直下の `docs/explain-forhuman.md` として追加する。
...
この文書は、既存の `README.md` / `docs/command.md` / `docs/research/latest/README.md` の**代替**ではなく、  
...
### 実装内容
- `docs/explain-forhuman.md` に以下を整理する
  1. 最初に結論
...
### In Scope
- `docs/explain-forhuman.md` の新規追加
...
- `README.md` / `docs/command.md` / `docs/research/latest/README.md` の改稿
...
**判定:** ファイル衝突は低い。今回は原則 `docs/explain-forhuman.md` の新規追加のみ。
...
### GREEN
- `docs/explain-forhuman.md` に必須項目をすべて記載する
...
- [ ] `docs/explain-forhuman.md` の章立てを作る
- [ ] `docs/explain-forhuman.md` を repo 直下に作成する
- [ ] 文体を非エンジニア向けに調整する
- [ ] diff / 表記 / 根拠を確認する
...
```bash
git --no-pager diff -- docs/explain-forhuman.md docs/exec-plans/active docs/exec-plans/completed
git --no-pager diff --check
rg -n "universe|campaign|preset|workflow|bundle-foreground-reuse-config|night-batch-self-hosted|README.md|docs/command.md|docs/research/latest/README.md|最短ルート" docs/explain-forhuman.md
